const NETWORK_CONDITIONS = {
  fast3g: { min: 500, max: 1_500 },
  slow3g: { min: 1_000, max: 3_000 },
  '2g': { min: 2_000, max: 5_000 },
};
...
export default class LazyImageLoader {
  ...
  #setupImage($img) {
    ...
    if (this.options.loadingIndicator) {
      ...
      this.#loadingStates.set($img, { spinner: $spinner });

      console.log('this.#loadingStates: ', this.#loadingStates);
    }
  ...
  #createLoadingSpinner() {
    ...
    if (!document.querySelector('#lazy-loading-spinner-keyframes')) {
      const $style = document.createElement('style');
      $style.id = 'lazy-loading-spinner-keyframes';
      $style.textContent = `
        @keyframes spin {
          0% {
